...
public class Trace {
    ...
    public static void trace(byte level, String s, Throwable t) {
        ...
        TernCorePlugin ternCorePlugin = TernCorePlugin.getDefault();
        if (level == SEVERE) {
            if (!logged.contains(s)) {
                if (ternCorePlugin != null) {
                    ternCorePlugin.getLog().log(new Status(IStatus.ERROR, TernCorePlugin.PLUGIN_ID, s, t));
                } else {
                    System.err.println("no tern plugin!");
                }
                logged.add(s);
            }
        }

        if (ternCorePlugin != null && !ternCorePlugin.isDebugging()) {
            return;
        }
